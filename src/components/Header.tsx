...
import { Home, ClipboardList, FlaskConical, LogOut, User, Menu, History, CalendarIcon, Shield } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const Header: React.FC<HeaderProps> = ({ title = "Contrôle Qualité Microbiologique", hideMenuItems }) => {
  ...
  const menuItems = [
    ...
    { icon: <CalendarIcon className="w-4 h-4 mr-2" />, label: 'Lectures en Attente', path: '/lectures-en-attente' },
    { icon: <FlaskConical className="w-4 h-4 mr-2" />, label: 'Historique', path: '/history' },
    { icon: <History className="w-4 h-4 mr-2" />, label: 'Formulaires', path: '/forms-history' },
    { icon: <Shield className="w-4 h-4 mr-2" />, label: 'Administration', path: '/admin' },
        ...
        <div className="flex items-center gap-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuItem className="flex items-center gap-2 text-sm">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-gray-500 text-xs capitalize">
                      {user.role === 'coordinator' ? 'Demandeur' : user.role === 'technician' ? 'Technicien' : user.role}
                    </span>
                  </div>
                </DropdownMenuItem>
              )}
              ...
              <DropdownMenuItem onClick={() => navigate('/forms-history')}>
                <History className="mr-2 h-4 w-4" />
                <span>Historique des formulaires</span>
              </DropdownMenuItem>
