...
const NotificationPanel = () => {
  ...
  const loadData = async () => {
    ...
    try {
      // Charger les analyses sauvegardées depuis Supabase (plus de localStorage)
      ...
      const fetchOngoingAnalyses = async () => {
        try {
          // Récupérer les analyses en cours depuis Supabase uniquement
          ...
          // Regrouper les échantillons par formulaire (report_title)
          ...
          setOngoingAnalyses(supabaseAnalyses);
          
          console.log('Analyses en cours récupérées:', supabaseAnalyses);
        } catch (error) {
  ...
  useEffect(() => {
    // Récupérer les données depuis Supabase au lieu du localStorage
    const fetchAnalysesFromDatabase = async () => {
      try {
        ...
        if (samples && samples.length > 0) {
          ...
          const recentForms = samples.reduce((acc, sample) => {
            const formId = sample.form_id || 'unknown';
            if (!acc[formId]) {
              acc[formId] = {
                formId,
                sampleCount: 0,
            ...
            acc[formId].sampleCount++;
            return acc;
          }, {});

          const formsList = Object.values(recentForms).slice(0, 3);
                            ...
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <div className="flex items-center gap-2">
                                <Badge variant={
                                  ...
                                  {analysis.creatorRole === 'coordinator' ? 'Créé par Demandeur' : 'En attente'}
                                </Badge>
                                <Badge variant={
